...
function arrangeCircle() {
  if (isAnimating) {
    var x2 = 1.5 * Math.PI- (prevCurrent - 1) * 2 * Math.PI / rowNumber -angle*direction;
  }
  else {
    var x2 = 1.5 * Math.PI- (current - 1) * 2 * Math.PI / rowNumber -angle*direction;
  }
  ...
  ctx.closePath();
  drawNodes();
  if (isAnimating&&angle<2*Math.PI / rowNumber) {
    drawCanvasLines();
  angle+=Math.PI/45;
  requestAnimationFrame(arrangeCircle);
  }
  else if(isAnimating){
    isAnimating=false;
    angle=0;
